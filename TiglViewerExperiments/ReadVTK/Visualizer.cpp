...
osg::Node* Visualizer::addCross(osg::ref_ptr<osgViewer::View> view,
...
Visualizer::Visualizer(int posX, int posY, int width, int height)
{
    ...
    root->addChild(addCross(views[0],0,0,width/3, height/3));


    TiglViewerBackground * bg = new TiglViewerBackground;
    //bg->makeGradient(osg::Vec4(0.6, 0.6, 1., 1.));
    bg->makeGradient(osg::Vec4(1., 235/255., 100/255., 1.));
    root->addChild(bg);
...
void Visualizer::addObject(char* filename, char* objectName)
{
    ...
    geode->readVTK(filename);
    geode->setName(objectName);
