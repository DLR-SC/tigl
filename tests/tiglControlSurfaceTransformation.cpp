...
TEST(TiglControlSurfaceTransformation, localTransformation)
{
    for ( int i = 0; i < 5; i++ )
    {
        ...
        if ( i == 0) {
            ...
            xyz3 = new gp_XYZ(1,-1024,0);
            xyz4 = new gp_XYZ(512,2,312);
        }
        ...
        ASSERT_EQ(p1Trans.X(),0.0);
        ASSERT_EQ(p1Trans.Y(),0.0);
        ASSERT_EQ(p1Trans.Z(),0.0);

        gp_Pnt p2Trans = p2.Transformed(t);
        ASSERT_NEAR(p2Trans.Y(),0.0,1e-7);
        ASSERT_NEAR(p2Trans.Z(),0.0,1e-7);
        /* only the x value of p2Trans can varry */
