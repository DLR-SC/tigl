...
CCPACSLeadingEdgeDevice::CCPACSLeadingEdgeDevice(
    CCPACSLeadingEdgeDevices *parent, CTiglUIDManager *uidMgr)
...
void CCPACSLeadingEdgeDevice::ReadCPACS(const TixiDocumentHandle &tixiHandle,
                                        const std::string &xpath) {
  CPACSTrailingEdgeDevice::ReadCPACS(tixiHandle, xpath);

  m_currentControlParam =
      Clamp(0., GetMinControlParameter(), GetMaxControlParameter());
}
...
gp_Trsf CCPACSLeadingEdgeDevice::GetFlapTransform() const {
  // this block of code calculates all needed values to rotate and move the
  // controlSurfaceDevice according to the given controlParal by using a
  // linearInterpolation.
  std::vector<double> controlParams, innerXTrans, outerXTrans, innerYTrans,
      innerZTrans, outerZTrans, rotations;
  ...
  double rotation =
      Interpolate(controlParams, rotations, m_currentControlParam);
  double innerTranslationX =
      Interpolate(controlParams, innerXTrans, m_currentControlParam);
  double innerTranslationY =
      Interpolate(controlParams, innerYTrans, m_currentControlParam);
  double innerTranslationZ =
      Interpolate(controlParams, innerZTrans, m_currentControlParam);
  double outerTranslationX =
      Interpolate(controlParams, outerXTrans, m_currentControlParam);
  double outerTranslationZ =
      Interpolate(controlParams, outerZTrans, m_currentControlParam);
  ...
  // innerTranslationY on hingePoint1 on purpose, maybe consider setting it to
  // zero as default. See CPACS definition on Path/Step/HingeLineTransformation
  // for more information.
  gp_Pnt outerHingeNew =
      outerHingeOld.XYZ() +
      gp_XYZ(outerTranslationX, innerTranslationY, outerTranslationZ);
  gp_Pnt innerHingeNew =
      innerHingeOld.XYZ() +
      gp_XYZ(innerTranslationX, innerTranslationY, innerTranslationZ);

  // calculating the needed transformations
  CTiglControlSurfaceTransformation transformation(
      innerHingeOld, outerHingeOld, innerHingeNew, outerHingeNew, rotation);
...
void CCPACSLeadingEdgeDevice::SetControlParameter(const double param) {
  ...
  double new_param =
      Clamp(param, GetMinControlParameter(), GetMaxControlParameter());
...
const CCPACSWingComponentSegment &
ComponentSegment(const CCPACSLeadingEdgeDevice &self) {
  return *self.GetParent()->GetParent()->GetParent();
}

CCPACSWingComponentSegment &ComponentSegment(CCPACSLeadingEdgeDevice &self) {
  return const_cast<CCPACSWingComponentSegment &>(
      ComponentSegment(const_cast<const CCPACSLeadingEdgeDevice &>(self)));
}
...
CCPACSWing &CCPACSLeadingEdgeDevice::Wing() {
  return const_cast<CCPACSWing &>(
      const_cast<const CCPACSLeadingEdgeDevice *>(this)->Wing());
}

void CCPACSLeadingEdgeDevice::ComputeHingePoints(
    CCPACSLeadingEdgeDevice::HingePoints &hingePoints) const {
  ...
  for (size_t i = 0; i < hingeXsi.size(); i++) {
    ...
    const CCPACSControlSurfaceBorderTrailingEdge &border =
        (i == 0 ? GetOuterShape().GetInnerBorder()
                : GetOuterShape().GetOuterBorder());

    if (relHingeHeight[i] < 0. || 1. < relHingeHeight[i]) {
      LOG(ERROR) << "The " << innerOuter
                 << "HingeHeight is not between 0. and 1.";
      throw CTiglError("The " + innerOuter +
                       "HingeHeight is not between 0. and 1. in "
                       "CCPACSLeadingEdgeDevice::buildHingePoints.");
    }
    if (hingeXsi[i] < 0. || 1. < hingeXsi[i]) {
      LOG(ERROR) << "The " << innerOuter << "HingeXsi is not between 0. and 1.";
      throw CTiglError("The " + innerOuter +
                       "HingeXsi is not between 0. and 1. in "
                       "CCPACSLeadingEdgeDevice::buildHingePoints.");
    }

    // create the hinge line point and normal on the Wing component segment mid
    // plane
    gp_Pnt myHingePoint =
        cSegment.GetPoint(transformEtaToCSOrTed(border.GetEtaLE(), *m_uidMgr),
                          hingeXsi[i], WING_COORDINATE_SYSTEM);
    gp_Vec midplaneNormal = cSegment.GetMidplaneNormal(
        transformEtaToCSOrTed(border.GetEtaLE(), *m_uidMgr));

    // Project point on the inner and outer face of the Wing loft
    gp_Pnt upper, lower;
    upper = ProjectPointOnShape(wsr.GetUpperShape(WING_COORDINATE_SYSTEM),
                                myHingePoint, midplaneNormal);
    lower = ProjectPointOnShape(wsr.GetLowerShape(WING_COORDINATE_SYSTEM),
                                myHingePoint, midplaneNormal);
    // then get delta through distance of these points multiplied by rel hinge
    // height
    ...
    auto translation =
        (i == 0 ? GetPath().GetInnerHingePoint().GetTranslation()
...
void CCPACSLeadingEdgeDevice::ComputeCutoutShape(PNamedShape &shape) const {
  if (!GetWingCutOut()) {
    ...
    shape = GetWingCutOut()->GetLoft(
        ComponentSegment(*this).GetWing().GetWingCleanShape(), GetOuterShape(),
        GetNormalOfControlSurfaceDevice());
  }
}

void CCPACSLeadingEdgeDevice::ComputeFlapShape(PNamedShape &shape) const {
  shape = GetOuterShape().GetLoft(Wing().GetWingCleanShape(),
                                  GetNormalOfControlSurfaceDevice());
...
CCPACSLeadingEdgeDevice::GetComponentIntent() const {
