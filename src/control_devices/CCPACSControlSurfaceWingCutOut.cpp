...
CCPACSControlSurfaceWingCutOut::CCPACSControlSurfaceWingCutOut(
    CCPACSTrailingEdgeDevice *parent, CTiglUIDManager *uidMgr)
    : generated::CPACSControlSurfaceWingCutOut(parent, uidMgr) {}

PNamedShape CCPACSControlSurfaceWingCutOut::GetLoft(
  ...
  TopoDS_Wire innerWire =
      GetCutoutWire(CCPACSControlSurfaceWingCutOut::CutoutPosition::InnerBorder,
                    wingCleanShape, &outerShape.GetInnerBorder(), upDir);
  TopoDS_Wire outerWire =
      GetCutoutWire(CCPACSControlSurfaceWingCutOut::CutoutPosition::OuterBorder,
                    wingCleanShape, &outerShape.GetOuterBorder(), upDir);
  ...
  PNamedShape cutout(
      new CNamedShape(thrusections.Shape(), GetParent()->GetUID().c_str()));
...
TopoDS_Wire CCPACSControlSurfaceWingCutOut::GetCutoutWire(
    CCPACSControlSurfaceWingCutOut::CutoutPosition pos,
    PNamedShape wingCleanShape,
  ...
  if (pos == CutoutPosition::InnerBorder) {
    if (!GetUpperSkin().GetXsiInnerBorder_choice2() ||
        !GetLowerSkin().GetXsiInnerBorder_choice2()) {
      throw CTiglError(
          "Inner Border xsi values must be set for upper and lower skin");
    }

    xsiUpper = GetUpperSkin().GetXsiInnerBorder_choice2().value();
    xsiLower = GetLowerSkin().GetXsiInnerBorder_choice2().value();
  } else {
    if (!GetUpperSkin().GetXsiOuterBorder_choice2() ||
        !GetLowerSkin().GetXsiOuterBorder_choice2()) {
      throw CTiglError(
          "Outer Border xsi values must be set for upper and lower skin");
    }
  ...
  if (lePoints) {
    const tigl::generated::CPACSCutOutControlPoint &lePoint =
        pos == CutoutPosition::InnerBorder ? lePoints->GetInnerBorder()
                                           : lePoints->GetOuterBorder();

    double xsiNose = lePoint.GetXsi();
    // TODO: calculate xsiNose into coordinate system of the border

    wire = builder.borderWithLEShape(lePoint.GetRelHeight(), xsiNose, xsiUpper,
                                     xsiLower);
  } else {
...
CCPACSControlSurfaceWingCutOut::GetCutoutCS(
    bool isInnerBorder,
    const CCPACSControlSurfaceBorderTrailingEdge *outerShapeBorder,
    const gp_Vec &upDir) const {
  ...
  if (!cutOutBorder->GetEtaLE_choice2() || !cutOutBorder->GetEtaTE_choice2()) {
    throw CTiglError("Cutout border of '" + GetParent()->GetUID() +
                     "' requires etaLE and etaTE values to proceed.");
  }

  double lEta = transformEtaToCSOrTed(cutOutBorder->GetEtaLE_choice2().value(),
                                      *m_uidMgr);
  double lXsi = transformXsiToCSOrTed(outerShapeBorder->GetXsiLE(), *m_uidMgr);
  double tEta = transformEtaToCSOrTed(cutOutBorder->GetEtaTE_choice2().value(),
                                      *m_uidMgr);
...
ComponentSegment(const CCPACSControlSurfaceWingCutOut &self) {
  if (!self.GetParent()) {
