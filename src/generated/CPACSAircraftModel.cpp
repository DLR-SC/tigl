...
namespace generated
{
    CPACSAircraftModel::CPACSAircraftModel(CPACSAircraft* parent, CTiglUIDManager* uidMgr)
        : m_uidMgr(uidMgr)
    {
        //assert(parent != NULL);
        m_parent = parent;
    ...
    const CPACSAircraft* CPACSAircraftModel::GetParent() const
    {
        return m_parent;
    }

    CPACSAircraft* CPACSAircraftModel::GetParent()
    {
        return m_parent;
    }

    const CTiglUIDObject* CPACSAircraftModel::GetNextUIDParent() const
    {
        if (m_parent) {
    ...
    CTiglUIDObject* CPACSAircraftModel::GetNextUIDParent()
    {
        if (m_parent) {
            return m_parent->GetNextUIDParent();
        }
    ...
    void CPACSAircraftModel::ReadCPACS(const TixiDocumentHandle& tixiHandle, const std::string& xpath)
    {
        ...
        if (tixi::TixiCheckElement(tixiHandle, xpath + "/engines")) {
            m_engines = boost::in_place(reinterpret_cast<CCPACSAircraftModel*>(this), m_uidMgr);
            try {
