// Copyright (c) 2020 RISC Software GmbH
//
// This file was generated by CPACSGen from CPACS XML Schema (c) German Aerospace Center (DLR/SC).
// Do not edit, all changes are lost when files are re-generated.
//
// Licensed under the Apache License, Version 2.0 (the "License")
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#include <cassert>
#include "CPACSCutOutProfiles.h"
#include "CPACSOuterCutOutProfile.h"
#include "CTiglError.h"
#include "CTiglLogging.h"
#include "CTiglUIDManager.h"
#include "CTiglUIDObject.h"
#include "TixiHelper.h"

namespace tigl
{
namespace generated
{
    CPACSOuterCutOutProfile::CPACSOuterCutOutProfile(CPACSCutOutProfiles* parent, CTiglUIDManager* uidMgr)
        : m_uidMgr(uidMgr)
        , m_rotZ(0)
    {
        //assert(parent != NULL);
        m_parent = parent;
    }

    CPACSOuterCutOutProfile::~CPACSOuterCutOutProfile()
    {
        if (m_uidMgr) {
            if (!m_profileUID.empty()) m_uidMgr->TryUnregisterReference(m_profileUID, *this);
        }
    }

    const CPACSCutOutProfiles* CPACSOuterCutOutProfile::GetParent() const
    {
        return m_parent;
    }

    CPACSCutOutProfiles* CPACSOuterCutOutProfile::GetParent()
    {
        return m_parent;
    }

    const CTiglUIDObject* CPACSOuterCutOutProfile::GetNextUIDParent() const
    {
        if (m_parent) {
            return m_parent->GetNextUIDParent();
        }
        return nullptr;
    }

    CTiglUIDObject* CPACSOuterCutOutProfile::GetNextUIDParent()
    {
        if (m_parent) {
            return m_parent->GetNextUIDParent();
        }
        return nullptr;
    }

    CTiglUIDManager& CPACSOuterCutOutProfile::GetUIDManager()
    {
        return *m_uidMgr;
    }

    const CTiglUIDManager& CPACSOuterCutOutProfile::GetUIDManager() const
    {
        return *m_uidMgr;
    }

    void CPACSOuterCutOutProfile::ReadCPACS(const TixiDocumentHandle& tixiHandle, const std::string& xpath)
    {
        // read element profileUID
        if (tixi::TixiCheckElement(tixiHandle, xpath + "/profileUID")) {
            m_profileUID = tixi::TixiGetElement<std::string>(tixiHandle, xpath + "/profileUID");
            if (m_profileUID.empty()) {
                LOG(WARNING) << "Required element profileUID is empty at xpath " << xpath;
            }
            if (m_uidMgr && !m_profileUID.empty()) m_uidMgr->RegisterReference(m_profileUID, *this);
        }
        else {
            LOG(ERROR) << "Required element profileUID is missing at xpath " << xpath;
        }

        // read element rotZ
        if (tixi::TixiCheckElement(tixiHandle, xpath + "/rotZ")) {
            m_rotZ = tixi::TixiGetElement<double>(tixiHandle, xpath + "/rotZ");
        }
        else {
            LOG(ERROR) << "Required element rotZ is missing at xpath " << xpath;
        }

    }

    void CPACSOuterCutOutProfile::WriteCPACS(const TixiDocumentHandle& tixiHandle, const std::string& xpath) const
    {
        const std::vector<std::string> childElemOrder = { "profileUID", "rotZ" };

        // write element profileUID
        tixi::TixiCreateSequenceElementIfNotExists(tixiHandle, xpath + "/profileUID", childElemOrder);
        tixi::TixiSaveElement(tixiHandle, xpath + "/profileUID", m_profileUID);

        // write element rotZ
        tixi::TixiCreateSequenceElementIfNotExists(tixiHandle, xpath + "/rotZ", childElemOrder);
        tixi::TixiSaveElement(tixiHandle, xpath + "/rotZ", m_rotZ);

    }

    const std::string& CPACSOuterCutOutProfile::GetProfileUID() const
    {
        return m_profileUID;
    }

    void CPACSOuterCutOutProfile::SetProfileUID(const std::string& value)
    {
        if (m_uidMgr) {
            if (!m_profileUID.empty()) m_uidMgr->TryUnregisterReference(m_profileUID, *this);
            if (!value.empty()) m_uidMgr->RegisterReference(value, *this);
        }
        m_profileUID = value;
    }

    const double& CPACSOuterCutOutProfile::GetRotZ() const
    {
        return m_rotZ;
    }

    void CPACSOuterCutOutProfile::SetRotZ(const double& value)
    {
        m_rotZ = value;
    }

    const CTiglUIDObject* CPACSOuterCutOutProfile::GetNextUIDObject() const
    {
        return GetNextUIDParent();
    }

    void CPACSOuterCutOutProfile::NotifyUIDChange(const std::string& oldUid, const std::string& newUid)
    {
        if (m_profileUID == oldUid) {
            m_profileUID = newUid;
        }
    }

} // namespace generated
} // namespace tigl
