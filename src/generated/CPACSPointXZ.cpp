...
namespace generated
{
    CPACSPointXZ::CPACSPointXZ(CCPACSControlSurfaceStep* parent, CTiglUIDManager* uidMgr)
        : m_uidMgr(uidMgr)
        , m_x(0)
        , m_z(0)
    {
    ...
    const CCPACSControlSurfaceStep* CPACSPointXZ::GetParent() const
    {
        return m_parent;
    }

    CCPACSControlSurfaceStep* CPACSPointXZ::GetParent()
    {
        return m_parent;
    }

    const CTiglUIDObject* CPACSPointXZ::GetNextUIDParent() const
    {
        if (m_parent) {
    ...
    CTiglUIDObject* CPACSPointXZ::GetNextUIDParent()
    {
        if (m_parent) {
            return m_parent->GetNextUIDParent();
        }
