...
namespace generated
{
    CPACSMaterial::CPACSMaterial(CPACSMaterials* parent, CTiglUIDManager* uidMgr)
        : m_uidMgr(uidMgr)
        , m_rho(0)
        , m_k11(0)
        , m_k12(0)
    ...
    const CPACSMaterials* CPACSMaterial::GetParent() const
    {
        return m_parent;
    }

    CPACSMaterials* CPACSMaterial::GetParent()
    {
        return m_parent;
    }

    const CTiglUIDObject* CPACSMaterial::GetNextUIDParent() const
    {
        if (m_parent) {
    ...
    CTiglUIDObject* CPACSMaterial::GetNextUIDParent()
    {
        if (m_parent) {
            return m_parent->GetNextUIDParent();
        }
    ...
    void CPACSMaterial::ReadCPACS(const TixiDocumentHandle& tixiHandle, const std::string& xpath)
    {
        ...
        if (tixi::TixiCheckElement(tixiHandle, xpath + "/postFailure")) {
            tixi::TixiReadElements(tixiHandle, xpath + "/postFailure", m_postFailures, 0, tixi::xsdUnbounded, this);
        }
        ...
        if (tixi::TixiCheckElement(tixiHandle, xpath + "/specificHeatMap")) {
            m_specificHeatMap = boost::in_place(this);
            try {
        ...
        if (tixi::TixiCheckElement(tixiHandle, xpath + "/emissivityMap")) {
            m_emissivityMap = boost::in_place(this);
            try {
    ...
    {
        m_postFailures.push_back(make_unique<CPACSPostFailure>(this));
