...
namespace generated
{
    CPACSRotorHubHinge::CPACSRotorHubHinge(CCPACSRotorHinges* parent, CTiglUIDManager* uidMgr)
        : m_uidMgr(uidMgr)
        , m_transformation(reinterpret_cast<CCPACSRotorHinge*>(this), m_uidMgr)
    {
        //assert(parent != NULL);
        m_parent = parent;
    }
    ...
    const CTiglUIDObject* CPACSRotorHubHinge::GetNextUIDParent() const
    {
        if (m_parent) {
    ...
    CTiglUIDObject* CPACSRotorHubHinge::GetNextUIDParent()
    {
        if (m_parent) {
            return m_parent->GetNextUIDParent();
        }
