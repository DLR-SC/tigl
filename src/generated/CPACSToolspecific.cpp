...
namespace generated
{
    CPACSToolspecific::CPACSToolspecific(CPACSCpacs* parent, CTiglUIDManager* uidMgr)
        : m_uidMgr(uidMgr)
    {
        //assert(parent != NULL);
        m_parent = parent;
    ...
    const CPACSCpacs* CPACSToolspecific::GetParent() const
    {
        return m_parent;
    }

    CPACSCpacs* CPACSToolspecific::GetParent()
    {
        return m_parent;
    }

    const CTiglUIDObject* CPACSToolspecific::GetNextUIDParent() const
    {
        if (m_parent) {
    ...
    CTiglUIDObject* CPACSToolspecific::GetNextUIDParent()
    {
        if (m_parent) {
            return m_parent->GetNextUIDParent();
        }
    ...
    CTiglUIDManager& CPACSToolspecific::GetUIDManager()
    {
        return *m_uidMgr;
    }

    const CTiglUIDManager& CPACSToolspecific::GetUIDManager() const
    {
        return *m_uidMgr;
    }
    ...
    void CPACSToolspecific::ReadCPACS(const TixiDocumentHandle& tixiHandle, const std::string& xpath)
    {
        ...
        if (tixi::TixiCheckElement(tixiHandle, xpath + "/cFD")) {
            m_cFD = boost::in_place(this, m_uidMgr);
            try {
