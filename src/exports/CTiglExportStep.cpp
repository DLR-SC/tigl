...
//     IFSelect_ReturnStatus status;
    ...
    for (int w = 1; w <= _config.GetWingCount(); w++)
    {
        ...
        for (int i=1; i <= wing.GetComponentSegmentCount(); i++)
        {
            ...
            for (int j = 1; j <= segment.GetSparSegmentCount(); j++) 
            {
                ...
                PNamedShape shape(new CNamedShape(sparSegmentGeometry, "Spar"));
                shapes.push_back(shape);

                // get spar Cap geometry splitted with ribs
                if (sparSegment.hasCaps())
                {
                    ...
                    PNamedShape shapeU(new CNamedShape(sparCapsGeometry, "Upper Spar Caps"));
                    shapes.push_back(shapeU);
                    ...
                    PNamedShape shapeL(new CNamedShape(sparCapsGeometry, "Lower Spar Caps"));
                ...
                if (sparSegment.HasStringerReinforcements())
                {
                    ...
                    PNamedShape shape(new CNamedShape(sparStringer, "Spar stringer"));
                    shapes.push_back(shape);
                }
                ...
                if (sparSegment.HasFramesReinforcements())
                {
                    ...
                    PNamedShape shape(new CNamedShape(sparFrames, "Spar Frames"));
                    shapes.push_back(shape);
                }
            ...
            for (int j = 1; j <= segment.GetRibsDefinitionCount(); j++) {
                ...
                PNamedShape shape(new CNamedShape(ribsGeometry, "Ribs"));
                shapes.push_back(shape);
                //get rib Caps geometry splitted with spars
                if (ribsDefinition.HasCaps())
                {
                    ...
                    PNamedShape shapeU(new CNamedShape(ribCapsGeometry, "Upper Rib Caps"));
                    shapes.push_back(shapeU);
                    ...
                    PNamedShape shapeL(new CNamedShape(ribCapsGeometry, "Lower Rib Caps"));
                    shapes.push_back(shapeL);
                }
                ...
                if (ribsDefinition.HasStringerReinforcements())
                {
                    ...
                    PNamedShape shape(new CNamedShape(ribStringer, "Rib stringer"));
                    shapes.push_back(shape);
                }
                ...
                if (ribsDefinition.HasFramesReinforcements())
                {
                    ...
                    PNamedShape shape(new CNamedShape(ribFrames, "Rib frames"));
                    shapes.push_back(shape);
            ...
            if (upperShell.HasStringer()) 
            {
                ...
                PNamedShape shape(new CNamedShape(upperStringerGeometry, "Upper wing stringer"));
                shapes.push_back(shape);
            ...
            if (lowerShell.HasStringer()) 
            {
                ...
                PNamedShape shape(new CNamedShape(lowerStringerGeometry, "Lower Wing Stringer"));
                shapes.push_back(shape);
            }
        }
    }
    ...
    for (int f = 1; f <= _config.GetFuselageCount(); f++)
    {
        ...
        for (int i = 1; i <= fuselage.GetSegmentCount(); i++)
        {
            ...
            for(int i = 1;i <= structure.GetDoorCount();i++)
            {
                ...
                if (exportDoorGeometry)
                {
                    ...
                    PNamedShape shape(new CNamedShape(doorShape, "Fuselage door"));
                    shapes.push_back(shape);
                }
        ...
        for(int i = 1;i <= structure.GetBulkheadCount();i++)            //PressureBulkHeads
        {
            ...
            TopoDS_Shape verticalReinforcementShape = pressureBulkhead->getSplittedVerticalReinforcementGeometry();     //pressureBulkheadVerticalReinforcements
            if (symmetryAxis != TIGL_NO_SYMMETRY) 
            {
                verticalReinforcementShape = tigl::CTiglCommon::mirrorShape(verticalReinforcementShape, symmetryAxis);
            }

            PNamedShape shapeVR(new CNamedShape(verticalReinforcementShape, "Bulkhead vertical reinforcements"));
            shapes.push_back(shapeVR);
            
            TopoDS_Shape horizontalReinforcementShape = pressureBulkhead->getSplittedHorizontalReinforcementGeometry();     //pressureBulkheadHorizontalReinforcements
            if (symmetryAxis != TIGL_NO_SYMMETRY) 
            {
                horizontalReinforcementShape = tigl::CTiglCommon::mirrorShape(horizontalReinforcementShape, symmetryAxis);
            }
            
            PNamedShape shapeHR(new CNamedShape(horizontalReinforcementShape, "Bulkhead horizontal reinforcements"));
            shapes.push_back(shapeHR);
            
            TopoDS_Shape bulkheadShape = pressureBulkhead->getGeometry();       //pressureBulkHead
            ...
            PNamedShape shape(new CNamedShape(bulkheadShape, "Bulkhead"));
            shapes.push_back(shape);
        ...
        for (int i = 1; i <= structure.GetFrameCount(); i++)                //frames
        {
            ...
            // TODO add possibility for 3d fuselage structure
            TopoDS_Shape frameShape = frame->GetSplitGeometry(!frame->GeomState_3D());

            if (symmetryAxis != TIGL_NO_SYMMETRY) 
            {
                frameShape = tigl::CTiglCommon::mirrorShape(frameShape, symmetryAxis);
            }
            PNamedShape shape(new CNamedShape(frameShape, "Fuselage frames"));
            shapes.push_back(shape);
        ...
        for (int i = 1; i <= structure.GetStringerCount(); i++)             //Stringers
        {
            ...
            // TODO add possibility for 3d fuselage structure
            TopoDS_Shape stringerShape = stringer->GetSplitGeometry(!stringer->GeomState_3D());

            if (symmetryAxis != TIGL_NO_SYMMETRY) 
            {
                stringerShape = tigl::CTiglCommon::mirrorShape(stringerShape, symmetryAxis);
            }
            
            PNamedShape shape(new CNamedShape(stringerShape, "Fuselage stringer"));
            shapes.push_back(shape);
        }
        for (int i = 1; i <= structure.GetCargoCrossBeamStrutCount(); i++)      //crossBeamStruct
        {
            // get the crossBeamStruct
            tigl::CCPACSCrossBeamStrut* crossBeamStrut =  structure.GetCargoCrossBeamStrut(i);
            // get the geometric structure of the crossBeamStruct
            // TODO add possibility for 3d fuselage structure
            TopoDS_Shape CBSshape = crossBeamStrut->getGeometry(!crossBeamStrut->GeomState_3D());

            if (symmetryAxis != TIGL_NO_SYMMETRY)
            {
                CBSshape = tigl::CTiglCommon::mirrorShape(CBSshape, symmetryAxis);
            }

            PNamedShape shape(new CNamedShape(CBSshape, "Fuselage Cross Beam Struts"));
            shapes.push_back(shape);
        ...
        for (int i = 1; i <= structure.GetLongFloorBeamCount(); i++)            //longFloorBeam
        {
            ...
            // TODO add possibility for 3d fuselage structure
            TopoDS_Shape LFBshape = longFloorBeam->getGeometry(!longFloorBeam->GeomState_3D());

            if (symmetryAxis != TIGL_NO_SYMMETRY) 
            {
                LFBshape = tigl::CTiglCommon::mirrorShape(LFBshape, symmetryAxis);
            }

            PNamedShape shape(new CNamedShape(LFBshape, "Fuselage Long Floor Beams"));
            shapes.push_back(shape);
        ...
        for (int i = 1; i <= structure.GetCargoCrossBeamCount(); i++)           //Crossbeam
        {
            ...
            // TODO add possibility for 3d fuselage structure
            TopoDS_Shape CBshape = crossBeam->getCuttedGeometry(!crossBeam->GeomState_3D());

            if (symmetryAxis != TIGL_NO_SYMMETRY) 
            {
                CBshape = tigl::CTiglCommon::mirrorShape(CBshape, symmetryAxis);
            }

            PNamedShape shape(new CNamedShape(CBshape, "Cargo cross beam"));
            shapes.push_back(shape);
        }
    }
...
//                     }
    ...
    for (int a = 0; a < vWings.size(); a++)
        for (int b = 0; b < vWings.size(); b++)
            ...
            // create Farfield and write to step file
            if (!HisssFile)
            {

                CFarField FField(_config);

                PNamedShape shape(new CNamedShape(FField.getFarField(nHalfModel), "FarField"));
                shapes.push_back(shape);
                ...
                if (nHalfModel)
                {
                    ...
                    if (vWings.size() > 0)
                    {
                        CCPACSWing& wing = _config.GetWing(1);
                        // [[CAS_AES]] added for symmetry
                        TiglSymmetryAxis symmetryAxis = wing.GetSymmetryAxis();

                        if (!((symmetryAxis != TIGL_NO_SYMMETRY) && !nHalfModel)) 
                        {
                            TopoDS_Compound compCutGeom;
                            builder.MakeCompound(compCutGeom);

                            TopoDS_Shape ffSymPlane = FField.getSymPlane(nHalfModel);
                            TopoDS_Compound compSymPlane;
                            builder.MakeCompound(compSymPlane);

                            builder.Add(compSymPlane, ffSymPlane);

                            double u_min = 0.0, u_max = 0.0, v_min = 0.0, v_max = 0.0;

                        // TODO: add FFS to cut symetry plane

                            for (int a = 0; a < vFuselages.size(); a++)
                            {
                                ffSymPlane = compSymPlane;

                                Bnd_Box bBox1, bBox2;
                                BRepBndLib::Add(ffSymPlane, bBox1);
                                BRepBndLib::Add(vFuselages[a].Solid, bBox2);
                                ...
                                TopoDS_Shape loft = CTiglCommon::splitShape(ffSymPlane, vFuselages[a].Solid);
                                faceMap.Clear();
                                TopExp::MapShapes(loft, TopAbs_FACE, faceMap);

                                BRepClass3d_SolidClassifier solidFuseClass(vFuselages[a].Solid);
                                ...
                                for (int f = 1; f <= faceMap.Extent(); f++) 
                                {
                                    ...
                                    if (solidFuseClass.State() == TopAbs_OUT)
                                    {
                                        builder.Add(compSymPlane, loftFace);
                                    }

                                    if (solidFuseClass.State() == TopAbs_UNKNOWN)
                                    {
                                        LOG(ERROR) << "Error during Symmetry plane - Fuselage cut.";
                                        throw CTiglError("Error during Symmetry plane - Fuselage cut.");
                                    }
                                }
                            }
                            ...
                            for (int b = 0; b < vWings.size(); b++)
                            {
                                ...
                                Bnd_Box bBox1, bBox2;
                                BRepBndLib::Add(ffSymPlane, bBox1);
                                BRepBndLib::Add(vWings[b].Solid, bBox2);
                                ...
                                double u_min = 0.0, u_max = 0.0, v_min = 0.0, v_max = 0.0;
                                TopoDS_Shape loft = CTiglCommon::splitShape(ffSymPlane, vWings[b].Solid);
                                faceMap.Clear();
                                TopExp::MapShapes(loft, TopAbs_FACE, faceMap);
                                ...
                                for (int f = 1; f <= faceMap.Extent(); f++) 
                                {
                                    ...
                                    if (solidFuseClass.State() == TopAbs_UNKNOWN)
                                    {
                                        LOG(ERROR) << "Error during Symmetry plane - Wing cut.";
                                        throw CTiglError("Error during Symmetry plane - Wing cut.");
                                    }
                                }
                            }
                            ...
                            PNamedShape shape(new CNamedShape(ffSymPlane, "SymmetryPlane"));
                            shapes.push_back(shape);
