  ...
  const GeomFill_FillingStyle theStyle) 
{
  ...
  if(nc != 3 && nc != 4) return NULL;
  if(theBaseCurve < 1 || theBaseCurve > nc) return NULL;
  ...
  for(i = 0; i <= 1; ++i) 
  {
    ...
    if(C1[i]->IsRational() || C1[i+2]->IsRational() ||
      ...
      for(j = 1; j <= nbk; ++j) {
        if(C1[i]->Multiplicity(j) == C1[i+2]->Multiplicity(j)) continue;
        SameDistribution = Standard_False;
        break;
      }
      //      
      if(SameDistribution) {
        ...
        for(j = 1; j <= nbk; ++j) {
          ...
          if(Abs(C1[i]->Knot(j) - C1[i+2]->Knot(j)) < err) continue;
          SameDistribution = Standard_False;
          break;
        }
      }
