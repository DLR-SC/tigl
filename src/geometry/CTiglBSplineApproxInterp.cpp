...
void CTiglBSplineApproxInterp::InterpolatePoint(size_t pointIndex, bool withKink)
{
    ...
    for (Standard_Integer i = 0; i < m_indexOfInterpolated.size(); ++i) {
        if (m_indexOfInterpolated[i] == 0)  first = true;
        if (m_indexOfInterpolated[i] == m_pnts.Length()-1)  last = true;
    }
    if (first && last) first_and_last = true;
}
...
CTiglApproxResult CTiglBSplineApproxInterp::solve(const std::vector<double>& params, const TColStd_Array1OfReal& knots, const TColStd_Array1OfInteger& mults) const
{
    ...
    if(is_closed && C2_continuous) {
        contin_cons = 3;
        if(first_and_last) contin_cons = 2;
    }
