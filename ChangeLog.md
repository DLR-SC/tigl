...
2025/09/26
-General changes
  - Build system: migrated TiGL's build dependencies from the `dlr-sc` conda channel to `conda-forge`. OpenCASCADE is now the stock, unpatched conda-forge `occt` package (dynamically linked, `novtk` build; the `occt-static` build option has been removed), bumped from 7.6.2 to 7.8.1 (unavoidable: conda-forge's `occt` 7.6.2 only has a vtk-enabled build, whose dependencies conflict with the `tixi` package on every platform; `novtk` builds only exist from occt 7.7.2 onward). TiXI is consumed as conda-forge's `tixi` package. Two tests are disabled because their golden values no longer match under conda-forge's occt (`creatorWing.DISABLED_wingCreateSectionInsideWithParam`, `WingSegmentGuideCurves.DISABLED_tiglWingGetSegmentUpperSurfaceAreaTrimmed`) -- most likely because conda-forge's OCCT lacks the G2-continuous Coons-patch patch the previous dlr-sc build had, though this wasn't fully isolated from the version bump itself.
  - Build system: since conda-forge's `pythonocc-core` package doesn't ship the SWIG interface sources TiGL's Python bindings need (unlike the dlr-sc build), those are now vendored as a git submodule at `thirdparty/pythonocc-core`, pinned to match the `pythonocc-core`/`occt` version in `pixi.toml`. **TODO**: remove this vendoring and go back to consuming the sources straight from the conda package once [conda-forge/pythonocc-feedstock](https://github.com/conda-forge/pythonocc-feedstock) ships them.
  - Build system: the MATLAB SDK headers/import-libs previously supplied by the dlr-sc `matlab-libs` conda package (no conda-forge equivalent exists) are now vendored at `thirdparty/matlab-sdk/{win-64,osx-64}`, used automatically by `cmake/FindMATLAB.cmake` as a fallback when no real MATLAB installation is configured. This lets CI's Windows build actually compile the MEX binary for the first time; note it still isn't included in the packaged NSIS/ZIP installer, since the `interfaces` CPack component it (and the Python/Fortran bindings) install under was already excluded from `CPACK_COMPONENTS_ALL` before this change -- a pre-existing packaging gap, not something introduced here.
  ...
  - TIGLCreator: Add a "Show Symmetry" checkbox to the display options for components with a CPACS symmetry axis (e.g. wings), to toggle visibility of the mirrored geometry in the 3D view without changing the CPACS symmetry attribute ([#1287](https://github.com/DLR-SC/tigl/issues/1287))
