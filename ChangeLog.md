...
-General changes
  ...
  - Build system: `pythonocc-core`'s SWIG interface sources (not shipped by the conda-forge package) are now vendored as a git submodule at `thirdparty/pythonocc-core`; `swig` pinned to `4.4.*` to match. **TODO**: drop the vendoring once [conda-forge/pythonocc-feedstock](https://github.com/conda-forge/pythonocc-feedstock) ships these sources.
  - Build system: `pixi run configure` (all environments) now runs `git submodule update --init --recursive` first, so a plain `git clone` followed by `pixi run -e <env> configure` works without an extra manual submodule step (needed for the `thirdparty/pythonocc-core` and `thirdparty/cpacs_gen` submodules). `cmake/FindPythonOCC.cmake` also now emits an explicit warning naming the missing submodule and the fix, for configure invocations outside of pixi.
