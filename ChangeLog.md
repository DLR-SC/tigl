...
27/07/2020
 ...
 - General changes:
   ...
   - Added Markos example2 as CPACS file
   - Added autogenerated classes for fuselage walls
   ...
   - Added first test for new algorithm
   - Added fuselage wall classes to bindings
   - Added fuselage wall example 1 cpacs file
   - Added link to github actions from CI badge
   - Added more tests to new algorithm
   - Added new algorithm ``CTiglInterpolatePointsWithKinks``
   - Added old tests for control surfaces
   - Added option to draw face boundaries in TiGl Viewer
   - Added potentially buggy wall example
   - Added some tests for the CPACS definition
   - Added test cases to reproduce bug
   - Added test for new reparametrization function
   - Added test for symmetry behaviour
   - Added topology functions
   - Added unittest for NormalizeAngleDeg
   - Auto generated code for new symmetry flags
   - Avoid displaying misleading error messages
   - CI: Retry downloading files on failure
   ...
   - Changes accoring to my code review
   - Code cleanup
   - Code simplification
   ...
   - First implementation of Markos' wall code
   - First working flap code
   - First working version of kink modeling
   - Flaps are now drawn everywhere the same
   - Further reuse of new functions
   ...
   - Improvements of cmake coverage script
   - Made wall segments a geometric component
   - Make code compatible with newer OCCT versions
   ...
   - Merge pull request #674 from DLR-SC/jk/gh-ci
   - Merge pull request #682 from DLR-SC/jk/681/pr-to-update-website
   - Merge pull request #683 from DLR-SC/jk/add-ffd-curve-network-test
   - Merge pull request #686 from DLR-SC/generated_systems
   - Merge pull request #688 from DLR-SC/jk/genericSystems_unittests
   - Merge pull request #691 from DLR-SC/jk/ci-ccache
   ...
   - Merge pull request #701 from DLR-SC/700-prevent-hard-crash-in-ctiglpatchshell
   - Merge pull request #703 from DLR-SC/702-new_symmetry_flags
   - Merge pull request #708 from DLR-SC/707-fix_buildface_regression
   - Merge pull request #711 from DLR-SC/710-tixi31-fail
   - Minor test improvement
   - More symmetry tests
   - More work on intersection algorithm
   - One more test
   - Pin libtixi3-dev to 3.0.3
   - Ported first large part of control surfaces code
   - Refactored CWireToCurve
   - Refactored angle normalization into own function
   - Refactoring for better cache usage
   - Regenerated code with cpacsgen
   - Remove Miniconda from Linux/MacOS Build Chain in Github CI (#677)
   - Removed deprecated functions
   - Removed empty custom flap classes
   - Rename of a function
   - Renamed *Defection to *ControlParameter in API
   - Renamed deflection to controlParameter
   ...
   - Some refactoring of flap draw dialog
   - Started implementing control surfaces
   - Started prototype for kink definition in profiles
   - Style change
   - Update Qt for macOS builds (#635)
   - Update codecov.yml
   - Update installation.md
   - Updated cpacs files to 3.1
   - Updated cpacs schema to 3.1
   - Updated schema to add walls
   - Updated the cpacs schema according to cpacs proposal
   ...
   - add ffd curve network to test suite
   - add performance test for ``tiglCheckPointInside``
   - add small unit tests for ``CTiglPatchShell``
   - add some small unit tests
   - add tolerance to bounding box check in IsPointInsideShape
   - add two more of Marko's examples
   - add unit test for fuselage walls
   - add unit test for issue #647
   - add unit test for missing frame position
   ...
   - added invalidation in wing symmetry setter, removed redundant invalidate-call in wing section element
   - added invalidation to setters in wall segment
   - added invalidation to setters of engine classes
   - added invalidation to setters of external objects
   - added invalidation to setters of fuselage classes
   - added missing includes in swig interface file
   - added required TIGL_EXPORT to InvalidateReferences  function
   - added test case for invalidation
   - added types and functions in ``CTiglUIDManager`` for reference handling and invalidation
   - adding testcase for bug #694
   - avoid hard crash in ``CTiglPatchShell``
   - classify point using fuselage shape not bounding box
   - clear bounding box when we reset the component
   - code cleanup
   - const some getters
   - create pr to update website on release
   - disabled warning caused by diamond inheritance
   - dont install ccache from HEAD
   - fixing compile errors of unit test sources
   ...
   - further refactoring of fuselage walls
   - generating classes for genericSystems
   - install qt for static builds as well
   ...
   - refactor: use BRepClass_FaceClassifier when cutting walls
   - refactor: use uid manager instead of exceptions to find components
   - relax tolerance in CMergeShapes
   - remove CPACS file that supposedly revealed a bug
   - remove excess white space and line breaks
   - remove outdated TODO comment
   - replace MakeFace w/ BuildFace
   - simple unit tests for generic systems
   - some more exceptions in ``CTiglPatchShell``
   - some refactoring
   - test ``tiglCheckPointInside`` with outside point only
   - throw an error in case of wrong number of frame positions
   - trim wall at start and end connection
   - unpin tixi version
   - updated cpacs_gen and regenerated classes for adding TIGL_EXPORT to functions inherited from ``ITiglUIDRefObject``
   - updated custom cpacs classes for changed interface from generated files
   - updated generated tigl classes
   ...
   - updating cpacs_tigl_gen submodule to matching branch
   - updating generated files using current generator
 ...
 - Fixes:
   ...
   - change ChangeLog back to rst, fix indentation in CI
 ...
 - TiGLViewer:
   ...
   - Back-ported flaps related ``tiglviewer`` code
