...
* Changed: $Id: TIGLCreatorDocument.cpp 226 2012-10-23 19:18:29Z martinsiggel $
*
...
#include "TIGLCreatorInputoutput.h"
...
#include "TIGLCreatorFuseDialog.h"
...
#define START_COMMAND()                                                                                                \
    TIGLCreatorScopedCommand command(app->getConsole());                                                               \
    Q_UNUSED(command);
...
TIGLCreatorDocument::TIGLCreatorDocument(TIGLCreatorWindow* parentWidget, DocumentId docId)
    : QObject(parentWidget)
    , m_flapsDialog(new TIGLCreatorSelectWingAndFlapStatusDialog(this, parentWidget))
...
TIGLCreatorDocument::~TIGLCreatorDocument()
{
    closeCpacsConfiguration();
    m_cpacsHandle = -1;
}

void TIGLCreatorDocument::writeToStatusBar(const QString& text)
{
    app->statusBar()->showMessage(text);
    qApp->processEvents();
}

void TIGLCreatorDocument::displayError(const QString& text, const QString& header)
{
    app->displayErrorMessage(text, header);
}

void TIGLCreatorDocument::displayTiglError(const QString& msg, TiglReturnCode ret)
{
    displayError(QString("%1 Error code: %2").arg(msg).arg(tiglGetErrorString(ret)), "TIGL Error");
}

char* TIGLCreatorDocument::qstringToCstring(const QString& text)
{
    return strdup((const char*)text.toLatin1());
}

void TIGLCreatorDocument::configurationModifiedSinceLastSave()
{
    modifiedSinceLastSave = true;
}

void TIGLCreatorDocument::configurationSaved()
{
    modifiedSinceLastSave = false;
}

bool TIGLCreatorDocument::isConfigurationModifiedSinceLastSave()
{
...
TiglReturnCode TIGLCreatorDocument::openCpacsConfigurationFromString(const std::string cpacsFileContent, const QString& config_uid)
{
    ...
    if (tixiRet == NOT_WELL_FORMED) {
        displayError(QString("Error in function <u>tixiImportFromString</u> when trying to open the template file. "
                             "File not well-formed in XML-style. Error code: %1")
                         .arg(tixiRet),
                     "TIXI Error");
    ...
    if (tixiRet != SUCCESS) {
        displayError(
            QString(
                "Error in function <u>tixiImportFromString</u> when trying to open the template file. Error code: %1")
                .arg(tixiRet),
            "TIXI Error");
        return TIGL_XML_ERROR;
    }
...
TiglReturnCode TIGLCreatorDocument::openCpacsConfigurationFromFile(const QString& fileName, const QString& config_uid)
{
    ...
    if (tixiRet != SUCCESS) {
        displayError(QString("Error in function <u>tixiOpenDocument</u> when opening <br>file <i>" + fileName +
                             "</i>. Error code: %1")
                         .arg(tixiRet),
                     "TIXI Error");
        return TIGL_XML_ERROR;
...
TiglReturnCode TIGLCreatorDocument::openCpacsConfiguration(TixiDocumentHandle& tixiHandle, const QString& fileName, const QString& config_uid)
{
    ...
    if (tiglRet != TIGL_SUCCESS) {
        ...
        m_cpacsHandle = -1;
        displayError(QString("<u>tiglOpenCPACSConfiguration</u> returned %1").arg(tiglGetErrorString(tiglRet)),
...
void TIGLCreatorDocument::closeCpacsConfiguration()
{
    if (m_cpacsHandle < 1) {
        return;
    }
 ...
 */
void TIGLCreatorDocument::updateConfiguration()
{
...
tigl::CCPACSConfiguration& TIGLCreatorDocument::GetConfiguration() const
{
    tigl::CCPACSConfigurationManager& manager = tigl::CCPACSConfigurationManager::GetInstance();
    return manager.GetConfiguration(m_cpacsHandle);
}

QString TIGLCreatorDocument::GetLoadedConfigurationFileName() const
{
    return loadedConfigurationFileName;
}

void TIGLCreatorDocument::SetLoadedConfigurationFileName(const QString& configurationFileName)
{
...
QString TIGLCreatorDocument::dlgGetWingOrRotorBladeSelection()
{
    QStringList wings;
    bool ok;
    ...
    QString choice =
        QInputDialog::getItem(app, tr("Select Wing or Rotor Blade"), tr("Available Wings:"), wings, 0, false, &ok);
    if (ok) {
        return choice;
    }
    else {
...
QString TIGLCreatorDocument::dlgGetWingSelection(const QString& wingUid)
{
    QStringList wings;
    bool ok;
...
QString TIGLCreatorDocument::dlgGetWingComponentSegmentSelection(const QString& wingUID)
{
    ...
    QString choice = QInputDialog::getItem(app, tr("Select Component Segment"), tr("Available Component Segments:"),
                                           compSegs, 0, false, &ok);
    if (ok) {
        return choice;
    }
...
QString TIGLCreatorDocument::dlgGetWingSegmentSelection()
{
    QStringList segs;
    bool ok;
...
// Wing profile Dialog
QString TIGLCreatorDocument::dlgGetWingProfileSelection()
{
    ...
    QString choice = QInputDialog::getItem(app, tr("Select Wing Profile"), tr("Available Wing Profiles:"), wingProfiles,
                                           0, false, &ok);
    if (ok) {
        return choice;
    }
...
QString TIGLCreatorDocument::dlgGetRotorBladeSelection(const QString& rotorBladeUid)
{
    ...
    QString choice =
        QInputDialog::getItem(app, tr("Select Rotor Blade"), tr("Available Rotor Blades:"), wings, 0, false, &ok);
    if (ok) {
        return choice;
    }
    else {
...
QString TIGLCreatorDocument::dlgGetRotorBladeSegmentSelection()
{
    QStringList segs;
    bool ok;
...
QString TIGLCreatorDocument::dlgGetRotorProfileSelection()
{
    QStringList wingProfiles;
    bool ok;
    ...
    QString choice = QInputDialog::getItem(app, tr("Select Rotor Profile"), tr("Available Rotor Profiles:"),
                                           wingProfiles, 0, false, &ok);
    if (ok) {
        return choice;
    }
    else {
...
QString TIGLCreatorDocument::dlgGetFuselageSelection(const QString& Uid)
{
    ...
    QString choice =
        QInputDialog::getItem(app, tr("Select Fuselage"), tr("Available Fuselages:"), fuselages, 0, false, &ok);
    if (ok) {
        return choice;
    }
    else {
...
QString TIGLCreatorDocument::dlgGetFuselageSegmentSelection()
{
    QStringList segs;
    bool ok;
...
QString TIGLCreatorDocument::dlgGetFuselageProfileSelection()
{
    ...
    QString choice = QInputDialog::getItem(app, tr("Select Fuselage Profile"), tr("Available Fuselage Profiles:"),
                                           fuselageProfiles, 0, false, &ok);
    if (ok) {
        return choice;
    }
    else {
...
void TIGLCreatorDocument::drawComponentByUID(const QString& uid)
{
    typedef void (TIGLCreatorDocument::*Callback)(const QString&);
    ...
    try {
        ...
        if (GetConfiguration().GetUIDManager().HasGeometricComponent(uid.toStdString())) {
            ...
            if (!app->getScene()->GetShapeManager(m_docId).HasShapeEntry(uid.toStdString())) {
                ...
                if (loft) {
                    ...
                    auto shape = app->getScene()->displayShape(loft, m_docId, true, getDefaultShapeColor(), opacity, shaded);
                ...
                IObjectList objects = app->getScene()->GetShapeManager(m_docId).GetIObjectsFromShapeName(uid.toStdString());
                if (objects.empty()) {
...
void TIGLCreatorDocument::drawControlPointNetByUID(const QString& uid)
{
    ...
    try {
        ...
        for (int i = 0; i < loft->GetFaceCount(); ++i) {
            ...
            app->getScene()->displayShape(control_point_net, m_docId, false, Quantity_NOC_YELLOW);
        }
 ...
 */
void TIGLCreatorDocument::drawWingGuideCurves(tigl::CCPACSWing& wing)
{
    START_COMMAND()
    ...
    for (; anIter.More(); anIter.Next()) {
        const TopoDS_Shape& wire = anIter.Value();
        app->getScene()->displayShape(wire, m_docId, Standard_False, Quantity_NOC_RED);
    }
    ...
    for (const auto& point : points) {
        app->getScene()->displayPoint(point, "", m_docId, Standard_False, 0, 0, 0, 1.);
    }
...
{
    displayedShapeNames = app->getScene()->GetShapeManager(m_docId).GetDisplayedShapeNames();
    removeAirfoil();
    ...
    auto wireObj = app->getScene()->displayShape(wire, m_docId, true, getDefaultShapeColor());
    app->getScene()->GetShapeManager(m_docId).addObject(std::string("airfoil"), wireObj);
    ...
    if (profile.GetPointList_choice1()) {
        ...
        if (points.size() < 15) {
            for (unsigned int i = 0; i < points.size(); ++i) {
                ...
                app->getScene()->displayPoint(pnt, str.str().c_str(), m_docId, Standard_False, 0., 0., 0., 6.);
            }
        }
        else {
            for (double zeta = 0.0; zeta <= 1.0; zeta += 0.1) {
                try {
                    ...
                    text << "PT(" << zeta << ")";
                    app->getScene()->displayPoint(wirePoint, const_cast<char*>(text.str().c_str()), m_docId, Standard_False, 0.0, 0.0, 0.0, 2.0);
                    text.str("");
                }
                catch (tigl::CTiglError& ex) {
                    displayError(ex.what());
...
void TIGLCreatorDocument::drawWing(const QString& Uid, bool drawFlaps)
{
    ...
    auto objects = app->getScene()->GetShapeManager(m_docId).GetIObjectsFromShapeName(wingUid.toStdString());
    for (auto& obj : objects) {
        app->getScene()->GetShapeManager(m_docId).removeObject(obj);
...
bool TIGLCreatorDocument::drawWingFlaps(tigl::CCPACSWing& wing)
{
    try {
        if (!wing.GetComponentSegments()) {
            displayError(QString("The wing %1 does not have any control surfaces.").arg(wing.GetUID().c_str()),
                         "Error");
            return false;
        }
        ...
        auto shape = app->getScene()->displayShape(wing.GetLoftWithCutouts(), m_docId, true, getDefaultShapeColor());
        app->getScene()->GetShapeManager(m_docId).addObject(wing.GetUID(), shape);
        ...
        if (mirroredLoft) {
            auto shape = app->getScene()->displayShape(mirroredLoft, m_docId, true, getDefaultShapeSymmetryColor());
            app->getScene()->GetShapeManager(m_docId).addObject(wing.GetUID(), shape);
        }
        ...
        for (auto& pcs : wing.GetComponentSegments()->GetComponentSegments()) {
            ...
            if (auto& teds = pcs->GetControlSurfaces()->GetTrailingEdgeDevices()) {
                for (auto& ted : teds->GetTrailingEdgeDevices()) {
                    auto objs = app->getScene()->GetShapeManager(m_docId).GetIObjectsFromShapeName(ted->GetUID());
            ...
            if (auto& leds = pcs->GetControlSurfaces()->GetLeadingEdgeDevices()) {
                for (auto& led : leds->GetLeadingEdgeDevices()) {
                    auto objs = app->getScene()->GetShapeManager(m_docId).GetIObjectsFromShapeName(led->GetUID());
                    // remove old wireframe flap shapes
                    for (auto& obj : objs) {
                        app->getScene()->getContext()->Remove(obj, Standard_True);
                        app->getScene()->GetShapeManager(m_docId).removeObject(obj);
...
void TIGLCreatorDocument::drawWingFlap(const QString& uid)
{
    try {
        ...
        if (*obj.type == typeid(tigl::CCPACSTrailingEdgeDevice)) {
            ...
            app->getScene()->displayShape(ted->GetLoft(), m_docId, false, Quantity_NOC_GREEN);
            ...
            if (mirrored_loft) { 
                PNamedShape mirror_named(new CNamedShape(ted->GetLoft()->Shape(), (ted->GetUID() + ":mirrored").c_str()));
                auto mirror_shape = app->getScene()->displayShape(mirror_named, m_docId, false, Quantity_NOC_GREEN);
                app->getScene()->GetShapeManager(m_docId).addObject(ted->GetUID(), mirror_shape);
            }
            ...
            app->getScene()->displayShape(led->GetLoft(), m_docId, false, Quantity_NOC_GREEN);

             // if flap has mirrored loft, display it too (not mirrored), to later update its transform and then mirror it
            PNamedShape mirrored_loft = led->GetMirroredLoft();
            if (mirrored_loft) { 
                PNamedShape mirror_named(new CNamedShape(led->GetLoft()->Shape(), (led->GetUID() + ":mirrored").c_str()));
                auto mirror_shape = app->getScene()->displayShape(mirror_named, m_docId, false, Quantity_NOC_GREEN);
                app->getScene()->GetShapeManager(m_docId).addObject(led->GetUID(), mirror_shape);
...
void TIGLCreatorDocument::updateFlapTransform(const std::string& controlUID)
{
    ...
    if (*obj.type == typeid(tigl::CCPACSTrailingEdgeDevice)) {
        ...
        try {
            ...
            flaps =
                app->getScene()->GetShapeManager(m_docId).GetIObjectsFromShapeName(controlSurfaceDevice->GetUID());
            mflaps =
                app->getScene()->GetShapeManager(m_docId).GetIObjectsFromShapeName(controlSurfaceDevice->GetUID()+":mirrored");
        }
        catch (...) {
...
void TIGLCreatorDocument::removeWing(const QString& Uid)
{
    ...
    auto objects = app->getScene()->GetShapeManager(m_docId).GetIObjectsFromShapeName(wing.GetUID());
    for (auto& obj : objects) {
        app->getScene()->getContext()->Remove(obj, Standard_False);
        app->getScene()->GetShapeManager(m_docId).removeObject(obj);
    }
...
void TIGLCreatorDocument::removeFuselage(const QString& Uid)
                {
                    ...
                    auto objects = app->getScene()->GetShapeManager(m_docId).GetIObjectsFromShapeName(fuselage.GetUID());
                    for (auto& obj : objects) {
                        app->getScene()->getContext()->Remove(obj, Standard_False);
                        app->getScene()->GetShapeManager(m_docId).removeObject(obj);
                    }
...
void TIGLCreatorDocument::drawFuselage(const QString& Uid)
{
    ...
    for (int i = 1; i <= fuselage.GetSegmentCount(); i++) {
        ...
        app->getScene()->displayShape(segment.GetLoft(), m_docId, true, getDefaultShapeColor());
    }
}
...
void TIGLCreatorDocument::drawFuselageTriangulation(const QString& Uid)
{
    ...
    auto shape = app->getScene()->displayShape(triangulation, m_docId, true, getDefaultShapeColor());
    app->getScene()->GetShapeManager(m_docId).addObject(fuselageUid.toStdString(), shape);
}
    ...
    for (int segmentIndex = 1; segmentIndex <= fuselage.GetSegmentCount(); segmentIndex++) {
        ...
        for (double eta = 0.0; eta <= 1.0; eta += 0.25) {
            for (double zeta = 0.0; zeta <= 1.0; zeta += 0.1) {
                ...
                auto points = app->getScene()->displayPoint(gp_Pnt(x, y, z), "", m_docId, false, 0, 0, 0, 1.0);
                app->getScene()->GetShapeManager(m_docId).addObject(fuselageUid.toStdString(), points);
            }
        }
    }
    app->getScene()->updateViewer();
...
void TIGLCreatorDocument::drawFuselageSamplePointsAngle(const QString& Uid)
{
    ...
    bool ok = false;
    double angle =
        QInputDialog::getDouble(nullptr, tr("Choose angle"), tr("Angle [degree]:"), 45., -360., 360., 1, &ok);
    if (!ok) {
        return;
    }
    ...
    for (int i = 1; i <= fuselage.GetSegmentCount(); i++) {
        ...
        auto points = app->getScene()->displayPoint(gp_Pnt(x, y, z), "", m_docId, Standard_False, 0., 0., 0., 1.);
        app->getScene()->GetShapeManager(m_docId).addObject(fuselageUid.toStdString(), points);
    }
    app->getScene()->updateViewer();
}
...
void TIGLCreatorDocument::drawAllFuselagesAndWingsSurfacePoints()
{
    ...
    for (int wingIndex = 1; wingIndex <= GetConfiguration().GetWingCount(); wingIndex++) {
        ...
        app->getScene()->displayShape(wing.GetLoft(), m_docId, true, getDefaultShapeColor());
        ...
        for (int segmentIndex = 1; segmentIndex <= wing.GetSegmentCount(); segmentIndex++) {
            for (double eta = 0.0; eta <= 1.0; eta += 0.1) {
                for (double xsi = 0.0; xsi <= 1.0; xsi += 0.1) {
                    double x, y, z;
                    tiglWingGetUpperPoint(m_cpacsHandle, wingIndex, segmentIndex, eta, xsi, &x, &y, &z);

                    app->getScene()->displayPoint(gp_Pnt(x, y, z), "", m_docId, Standard_False, 0, 0, 0, 1.);

                    tiglWingGetLowerPoint(m_cpacsHandle, wingIndex, segmentIndex, eta, xsi, &x, &y, &z);
    ...
    for (int fuselageIndex = 1; fuselageIndex <= GetConfiguration().GetFuselageCount(); fuselageIndex++) {
        ...
        app->getScene()->displayShape(fuselage.GetLoft(), m_docId, true, getDefaultShapeColor());
        ...
        for (int segmentIndex = 1; segmentIndex <= fuselage.GetSegmentCount(); segmentIndex++) {
            ...
            for (double eta = 0.0; eta <= 1.0; eta += 0.25) {
                for (double zeta = 0.0; zeta <= 1.0; zeta += 0.1) {
                    double x, y, z;
                    tiglFuselageGetPoint(m_cpacsHandle, fuselageIndex, segmentIndex, eta, zeta, &x, &y, &z);
...
// Export Functions
// -----------------------
void TIGLCreatorDocument::exportAsIges()
{
    QString fileName;

    TIGLCreatorInputOutput writer;
...
void TIGLCreatorDocument::exportFusedAsIges()
{
    QString fileName;
    QString fileType;
...
void TIGLCreatorDocument::exportAsStep()
{
    QString fileName;
...
void TIGLCreatorDocument::exportAsStepFused()
{
    QString fileName;
...
void TIGLCreatorDocument::exportMeshedWingSTL()
{
    QString fileName;
    ...
    if (!fileName.isEmpty()) {
        ...
        double deflection      = wing.GetWingspan() / 2. * TIGLCreatorSettings::Instance().triangulationAccuracy();
        TiglReturnCode err     = tiglExportMeshedWingSTLByUID(m_cpacsHandle, qstringToCstring(wingUid),
                                                              qstringToCstring(fileName), deflection);
        if (err != TIGL_SUCCESS) {
...
void TIGLCreatorDocument::exportMeshedFuselageSTL()
{
    QString fileName;
    ...
    if (!fileName.isEmpty()) {
        START_COMMAND()
        TiglReturnCode err =
            tiglExportMeshedFuselageSTLByUID(m_cpacsHandle, qstringToCstring(fuselageUid), qstringToCstring(fileName),
                                             TIGLCreatorSettings::Instance().triangulationAccuracy());
        if (err != TIGL_SUCCESS) {
...
void TIGLCreatorDocument::exportMeshedConfigSTL()
{
    QString fileName;
    ...
    if (!fileName.isEmpty()) {
        START_COMMAND()
        TiglReturnCode err = tiglExportMeshedGeometrySTL(m_cpacsHandle, qstringToCstring(fileName),
                                                         TIGLCreatorSettings::Instance().triangulationAccuracy());
        if (err != TIGL_SUCCESS) {
...
void TIGLCreatorDocument::exportMeshedWingVTK()
{
    QString fileName;
    ...
    if (1) {
        START_COMMAND()
        tigl::CCPACSWing& wing = GetConfiguration().GetWing(qstringToCstring(wingUid));
        deflection             = wing.GetWingspan() / 2. * TIGLCreatorSettings::Instance().triangulationAccuracy();
    }

    TIGLCreatorVTKExportDialog settings(app);
    settings.setDeflection(deflection);
    settings.exec();

    if (1) {
        START_COMMAND()
        TiglReturnCode err = tiglExportMeshedWingVTKByUID(m_cpacsHandle, wingUid.toStdString().c_str(),
                                                          qstringToCstring(fileName), settings.getDeflection());
        if (err != TIGL_SUCCESS) {
...
void TIGLCreatorDocument::exportMeshedWingVTKsimple()
{
    QString fileName;
    TIGLCreatorInputOutput writer;
    ...
    if (1) {
        START_COMMAND()
        TiglReturnCode err = tiglExportMeshedWingVTKSimpleByUID(m_cpacsHandle, qstringToCstring(wingUid),
                                                                qstringToCstring(fileName), deflection);
        if (err != TIGL_SUCCESS) {
...
void TIGLCreatorDocument::exportWingCollada()
{
    QString fileName;
    ...
    if (!fileName.isEmpty()) {
        ...
        TiglReturnCode err = tiglExportWingColladaByUID(m_cpacsHandle, wingUid.toStdString().c_str(),
                                                        qstringToCstring(fileName), deflection);
        if (err != TIGL_SUCCESS) {
...
void TIGLCreatorDocument::exportFuselageCollada()
{
    QString fileName;
    ...
    if (!fileName.isEmpty()) {
        START_COMMAND()
        double deflection =
            GetConfiguration().GetAirplaneLength() * TIGLCreatorSettings::Instance().triangulationAccuracy();
        TiglReturnCode err = tiglExportFuselageColladaByUID(m_cpacsHandle, qstringToCstring(fuselageUid),
                                                            qstringToCstring(fileName), deflection);
        if (err != TIGL_SUCCESS) {
...
void TIGLCreatorDocument::exportConfigCollada()
{
...
void TIGLCreatorDocument::exportMeshedFuselageVTK()
{
    QString fileName;
    ...
    if (1) {
        START_COMMAND()
        deflection = GetConfiguration().GetAirplaneLength() * TIGLCreatorSettings::Instance().triangulationAccuracy();
    }
    ...
    if (settings.exec()) {
        ...
        START_COMMAND()
        TiglReturnCode err = tiglExportMeshedFuselageVTKByUID(m_cpacsHandle, wingUid.toStdString().c_str(),
                                                              qstringToCstring(fileName), settings.getDeflection());
        if (err != TIGL_SUCCESS) {
...
void TIGLCreatorDocument::exportMeshedConfigVTK()
{
    QString fileName;
    fileName = QFileDialog::getSaveFileName(app, tr("Save as..."), myLastFolder, tr("Export VTK(*.vtp)"));
...
void TIGLCreatorDocument::exportMeshedConfigVTKNoFuse()
{
    QString fileName;
    fileName = QFileDialog::getSaveFileName(app, tr("Save as..."), myLastFolder, tr("Export VTK(*.vtp)"));
...
void TIGLCreatorDocument::exportWingBRep()
{
    QString wingUid = dlgGetWingOrRotorBladeSelection();
    if (wingUid == "") {
        return;
    ...
    if (!fileName.isEmpty()) {
        START_COMMAND()
        TiglReturnCode err =
            tiglExportWingBREPByUID(m_cpacsHandle, qstringToCstring(wingUid), qstringToCstring(fileName));
        if (err != TIGL_SUCCESS) {
...
void TIGLCreatorDocument::exportFuselageBRep()
{
    ...
    if (!fileName.isEmpty()) {
        START_COMMAND()
        TiglReturnCode err =
            tiglExportFuselageBREPByUID(m_cpacsHandle, qstringToCstring(fuselageUid), qstringToCstring(fileName));
        if (err != TIGL_SUCCESS) {
...
void TIGLCreatorDocument::exportFusedConfigBRep()
{
    QString fileName = QFileDialog::getSaveFileName(app, tr("Save as..."), myLastFolder, tr("Export BRep(*.brep)"));
    if (fileName.isEmpty()) {
        return;
...
void TIGLCreatorDocument::exportWingCurvesBRep()
{
    QString wingUid = dlgGetWingOrRotorBladeSelection();
    if (wingUid == "") {
        return;
    ...
    for (int isegment = 1; isegment <= wing.GetSegmentCount(); ++isegment) {
        // get profiles
        auto& segment = static_cast<tigl::CCPACSWingSegment&>(wing.GetSegment(isegment));
...
void TIGLCreatorDocument::exportFuselageCurvesBRep()
{
    ...
    for (int isegment = 1; isegment <= fuselage.GetSegmentCount(); ++isegment) {
        // get profiles
        auto& segment = static_cast<tigl::CCPACSFuselageSegment&>(fuselage.GetSegment(isegment));
...
void TIGLCreatorDocument::drawFusedAircraft()
{
    ...
    try {
        ...
        int icol                      = 0;
        Quantity_NameOfColor colors[] = {Quantity_NOC_BLUE4,    Quantity_NOC_RED,   Quantity_NOC_GREEN,
                                         Quantity_NOC_MAGENTA1, Quantity_NOC_AZURE, Quantity_NOC_FIREBRICK};
        ...
        for (it = map.begin(); it != map.end(); ++it) {
            ...
            if (shape) {
                app->getScene()->displayShape(shape, m_docId, true, colors[icol++]);
            }
        }
        ...
        if (ff) {
            ...
            app->getScene()->getContext()->Display(shape, Standard_True), getDefaultShapeColor();
        }
    }
...
void TIGLCreatorDocument::drawIntersectionLine()
{
    ...
    TIGLCreatorShapeIntersectionDialog dialog(uidManager, app);
    if (dialog.exec() != QDialog::Accepted) {
        return;
    }
    ...
    tigl::CTiglIntersectionCalculation* Intersector = nullptr;
    if (mode == 0) {
        // shape - shape
        ...
        Intersector =
            new tigl::CTiglIntersectionCalculation(&config.GetShapeCache(), uid1, uid2, compoundOne, compoundTwo);
    }
    else if (mode == 1) {
        // shape - plane
    ...
    for (int wireID = 1; wireID <= Intersector->GetCountIntersectionLines(); ++wireID) {
        app->getScene()->displayShape(Intersector->GetWire(wireID), m_docId, true, getDefaultShapeColor());
...
void TIGLCreatorDocument::drawWingComponentSegment(const QString& Uid)
{
    ...
    auto objects = app->getScene()->GetShapeManager(m_docId).GetIObjectsFromShapeName(Uid.toStdString());
    for (auto& obj : objects) {
        app->getScene()->getContext()->Remove(obj, Standard_False);
        app->getScene()->GetShapeManager(m_docId).removeObject(obj);
    }
    ...
    auto cs_shape = app->getScene()->displayShape(cs.GetLoft(), m_docId, true, getDefaultShapeColor());
    app->getScene()->GetShapeManager(m_docId).addObject(Uid.toStdString(), cs_shape);
...
void TIGLCreatorDocument::drawWingStructure(const QString& Uid)
{
    ...
    try {
        ...
        auto& cs =
            GetConfiguration().GetUIDManager().ResolveObject<tigl::CCPACSWingComponentSegment>(csUid.toStdString());
        ...
        // display component segment shape with transparency
        auto cs_shape = app->getScene()->displayShape(cs.GetLoft(), m_docId, true, Quantity_NOC_ShapeCol, 0.5);
        app->getScene()->GetShapeManager(m_docId).addObject(WingUid.toStdString(), cs_shape);
        PNamedShape mirroredLoft = wing.GetMirroredLoft(cs.GetLoft()); 
            if (mirroredLoft)
            {
                auto shape = app->getScene()->displayShape(mirroredLoft, m_docId, true, Quantity_NOC_ShapeCol, 0.5);
                app->getScene()->GetShapeManager(m_docId).addObject(WingUid.toStdString(), shape);
        ...
        for (int ispar = 1; ispar <= structure.GetSparSegmentCount(); ++ispar) {
            ...
            auto spar_shape = app->getScene()->displayShape(sparGeom, m_docId, true, Quantity_NOC_RED);
            app->getScene()->GetShapeManager(m_docId).addObject(WingUid.toStdString(), spar_shape);

            PNamedShape loftNamed(new CNamedShape(sparGeom, WingUid.toStdString()));
            PNamedShape mirroredLoft = wing.GetMirroredLoft(loftNamed); 
            if (mirroredLoft)
            {
                auto shape = app->getScene()->displayShape(mirroredLoft, m_docId, true, Quantity_NOC_RED);
                app->getScene()->GetShapeManager(m_docId).addObject(WingUid.toStdString(), shape);
        ...
        for (int irib = 1; irib <= structure.GetRibsDefinitionCount(); ++irib) {
            ...
            auto rib_shape = app->getScene()->displayShape(ribGeom, m_docId, true, Quantity_NOC_RED);
            app->getScene()->GetShapeManager(m_docId).addObject(WingUid.toStdString(), rib_shape);
...
void TIGLCreatorDocument::drawFarField()
{
    START_COMMAND()
...
void TIGLCreatorDocument::drawSystems()
{
    ...
    for (int gs = 1; gs <= GetConfiguration().GetGenericSystemCount(); gs++) {
        ...
        try {
            app->getScene()->displayShape(genericSystem.GetLoft(), m_docId, true, getDefaultShapeColor());

            if (genericSystem.GetSymmetryAxis() != TIGL_NO_SYMMETRY) {
                app->getScene()->displayShape(genericSystem.GetMirroredLoft()->Shape(), m_docId, true, getDefaultShapeSymmetryColor());
            }
        }
...
void TIGLCreatorDocument::drawRotorByUID(const QString& uid)
{
    ...
    if (!app->getScene()->GetShapeManager(m_docId).HasShapeEntry(rotorUid.toStdString())) {
        // Draw segment loft
        app->getScene()->displayShape(rotor.GetLoft(), m_docId, true, Quantity_NOC_RotorCol);

        // Draw rotor disk
        TopoDS_Shape rotorDisk = rotor.GetRotorDisk()->Shape();
        auto shape = app->getScene()->displayShape(rotorDisk, m_docId, true, Quantity_NOC_RotorCol, 0.9);
        app->getScene()->GetShapeManager(m_docId).addObject(rotorUid.toStdString(), shape);
        ...
        // Draw mirrored rotor
        shape = app->getScene()->displayShape(rotor.GetMirroredLoft()->Shape(), m_docId, false, Quantity_NOC_MirrRotorCol);
        app->getScene()->GetShapeManager(m_docId).addObject(rotorUid.toStdString(), shape);
        ...
        shape = app->getScene()->displayShape(mirrRotorDisk, m_docId, true, Quantity_NOC_MirrRotorCol, 0.9);
        app->getScene()->GetShapeManager(m_docId).addObject(rotorUid.toStdString(), shape);
    }
    else {
        IObjectList objects = app->getScene()->GetShapeManager(m_docId).GetIObjectsFromShapeName(rotorUid.toStdString());
        for (auto& obj : objects) {
            app->getScene()->getContext()->Display(obj, Standard_False);
        }
        app->getScene()->getViewer()->Update();
...
void TIGLCreatorDocument::drawRotorDisk(const QString& uid)
{
    ...
    // Draw rotor disk
    TopoDS_Shape rotorDisk = rotor.GetRotorDisk()->Shape();
    app->getScene()->displayShape(rotorDisk, m_docId, true, Quantity_NOC_RotorCol, 0.9);
}
...
#define ADD_PROPERTY_TEXT(propertyName, text)                                                                          \
    ...
    propertiesText += QString("<b>%1:</b> %2<br/>").arg(propertyName).arg(valueText);
        ...
        ADD_PROPERTY_TEXT("Translation", "(" + QString::number(tmpPoint.x) + "; " + QString::number(tmpPoint.y) + "; " +
                                             QString::number(tmpPoint.z) + ")")
        ...
        if (rotor.GetRotorBladeCount() > 0) {
            ...
            ADD_PROPERTY_TEXT("Blade 1 Tip Radius", QString::number(blade.GetLocalRadius(
                                                        blade.GetUnattachedRotorBlade().GetSegmentCount(), 1.)));
            ADD_PROPERTY_TEXT("Blade 1 Root Chord", QString::number(blade.GetLocalChord(1, 0.)));
            ADD_PROPERTY_TEXT("Blade 1 Tip Chord", QString::number(blade.GetLocalChord(
                                                       blade.GetUnattachedRotorBlade().GetSegmentCount(), 1.)));

            ADD_PROPERTY_TEXT("Blade 1 Root Twist", QString::number(blade.GetLocalTwistAngle(1, 0.)));
            ADD_PROPERTY_TEXT("Blade 1 Tip Twist", QString::number(blade.GetLocalTwistAngle(
                                                       blade.GetUnattachedRotorBlade().GetSegmentCount(), 1.)));
        }
    }
...
Quantity_Color TIGLCreatorDocument::getDefaultShapeColor()
{
    QColor defaultShapeColor = app->getSettings().shapeColor();
    return Quantity_Color(defaultShapeColor.red() / 255., defaultShapeColor.green() / 255.,
                          defaultShapeColor.blue() / 255., Quantity_TOC_RGB);
}

Quantity_Color TIGLCreatorDocument::getDefaultShapeSymmetryColor()
{
    QColor defaultShapeColor = app->getSettings().shapeSymmetryColor();
    return Quantity_Color(defaultShapeColor.red() / 255., defaultShapeColor.green() / 255.,
                          defaultShapeColor.blue() / 255., Quantity_TOC_RGB);
}
...
void TIGLCreatorDocument::drawAirfoil(tigl::CCPACSWingProfile& profile)
{
    ...
    app->getScene()->displayPoint(lePoint, const_cast<char*>(text.str().c_str()), m_docId, Standard_False, 0.0, 0.0, 0.0, 2.0);
    text.str("");
    text << "TE(" << tePoint.X() << ", " << tePoint.Y() << ", " << tePoint.Z() << ")";
    app->getScene()->displayPoint(tePoint, const_cast<char*>(text.str().c_str()), m_docId, Standard_False, 0.0, 0.0, 0.0, 2.0);
    text.str("");
    ...
    if (length > 0.0) {
        ...
        app->getScene()->displayShape(le_te_edge, m_docId, true, getDefaultShapeSymmetryColor());
    }
    ...
    if (fewPointList.size() < 15 && !fewPointList.empty()) {
        ...
        for (double xsi = 0.1; xsi <= 0.9; xsi = xsi + 0.2) {
            try {
                ...
                app->getScene()->displayPoint(chordPoint, const_cast<char*>(text.str().c_str()), m_docId, Standard_False, 0.0, 0.0, 0.0, 2.0);
                text.str("");
            }
            catch (tigl::CTiglError& ex) {
                displayError(ex.what());
            ...
            try {
                ...
                app->getScene()->displayPoint(upperPoint, const_cast<char*>(text.str().c_str()), m_docId, Standard_False, 0.0, 0.0, 0.0, 2.0);
                text.str("");
            }
            catch (tigl::CTiglError& ex) {
                displayError(ex.what());
            ...
            try {
                ...
                app->getScene()->displayPoint(lowerPoint, const_cast<char*>(text.str().c_str()), m_docId, Standard_False, 0.0, 0.0, 0.0, 2.0);
                text.str("");
            }
            catch (tigl::CTiglError& ex) {
                displayError(ex.what());
...
void TIGLCreatorDocument::removeAirfoil()
{
    ...
    auto objects = app->getScene()->GetShapeManager(m_docId).GetIObjectsFromShapeName("airfoil");
    for (auto& obj : objects) {
        app->getScene()->getContext()->Remove(obj, Standard_False);
        app->getScene()->GetShapeManager(m_docId).removeObject(obj);
    }
...
void TIGLCreatorDocument::drawWingOverlayProfilePoints(tigl::CCPACSWing& wing)
{
    ...
    for (int i = 1; i <= wing.GetSegmentCount(); i++) {
        // Get segment
        auto& segment = (tigl::CCPACSWingSegment&)wing.GetSegment(i);
        ...
        for (const auto& j : innerProfilePointList) {

            gp_Pnt pnt = j.Get_gp_Pnt();
            pnt        = innerT.Transform(pnt);

            auto point = app->getScene()->displayPoint(pnt, "", m_docId, Standard_False, 0.0, 0.0, 0.0, 2.0);
            app->getScene()->GetShapeManager(m_docId).addObject(wing.GetUID(), point);
        }
        ...
        for (const auto& j : outerProfilePointList) {
 ...
 * Draw the input wing
 */
void TIGLCreatorDocument::drawWing(tigl::CCPACSWing& wing)
{
    START_COMMAND()
    ...
    for (int i = 1; i <= wing.GetSegmentCount(); i++) {
        // Draw segment loft
        app->getScene()->displayShape(wing.GetSegment(i).GetLoft(), m_docId, true, getDefaultShapeColor());
    }
}
 ...
 */
void TIGLCreatorDocument::drawWingTriangulation(tigl::CCPACSWing& wing)
{
    START_COMMAND()
    ...
    auto shape = app->getScene()->displayShape(compound, m_docId, true, getDefaultShapeColor());
    app->getScene()->GetShapeManager(m_docId).addObject(wing.GetUID(), shape);
 ...
 */
void TIGLCreatorDocument::drawWingSamplePoints(tigl::CCPACSWing& wing)
{
    START_COMMAND()
    ...
    for (int segmentIndex = 1; segmentIndex <= wing.GetSegmentCount(); segmentIndex++) {
        ...
        auto shape = app->getScene()->displayShape(segment.GetLoft(), m_docId, true, getDefaultShapeColor());
        app->getScene()->GetShapeManager(m_docId).addObject(wing.GetUID(), shape);
        ...
        for (double eta = 0.0; eta <= 1.0; eta += 0.1) {
            for (double xsi = 0.0; xsi <= 1.0; xsi += 0.1) {
                ...
                auto point = app->getScene()->displayPoint(gp_Pnt(x, y, z), "", m_docId, Standard_False, 0., 0., 0., 1.);
                app->getScene()->GetShapeManager(m_docId).addObject(wing.GetUID(), point);

                tiglWingGetLowerPoint(m_cpacsHandle, wingIndex, segmentIndex, eta, xsi, &x, &y, &z);

                point = app->getScene()->displayPoint(gp_Pnt(x, y, z), "", m_docId, Standard_False, 0., 0., 0., 1.);
                app->getScene()->GetShapeManager(m_docId).addObject(wing.GetUID(), point);
            }
        }
    }
 ...
 */
void TIGLCreatorDocument::drawFusedWing(tigl::CCPACSWing& wing)
{
    START_COMMAND()
    app->getScene()->deleteObjectsOfDocument(m_docId);
    app->getScene()->displayShape(wing.GetLoft(), m_docId, true, getDefaultShapeColor());
 ...
 */
void TIGLCreatorDocument::drawWingComponentSegment(tigl::CCPACSWingComponentSegment& segment)
{
    START_COMMAND()
    app->getScene()->deleteObjectsOfDocument(m_docId);
 ...
 */
void TIGLCreatorDocument::drawWingComponentSegmentPoint(const std::string& csUID, const double& eta, const double& xsi)
{
...
#else
    double x, y, z;
    TiglReturnCode ret = tiglWingComponentSegmentGetPoint(getCpacsHandle(), csUID.c_str(), eta, xsi, &x, &y, &z);
    if (ret == TIGL_SUCCESS) {
        gp_Pnt point(x, y, z);
        app->getScene()->displayPoint(point, "", m_docId, Standard_True, 0, 0, 0, 1.0);
    }
    else {
        displayError(QString("Error in <b>tiglWingComponentSegmentPointGetPoint</b>. ReturnCode: %1").arg(ret),
                     "Error");
    }
#endif
}
...
void TIGLCreatorDocument::drawWingShells(tigl::CCPACSWing& wing)
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        auto shape = app->getScene()->displayShape(shapes[i], m_docId, true, colors[i]);
        app->getScene()->GetShapeManager(m_docId).addObject(wing.GetUID(), shape);

        PNamedShape loftNamed(new CNamedShape(shapes[i], wing.GetUID().c_str()));
        PNamedShape mirroredLoft = wing.GetMirroredLoft(loftNamed);
        if (mirroredLoft) {
            auto shape = app->getScene()->displayShape(mirroredLoft, m_docId, true, colors[i]);
            app->getScene()->GetShapeManager(m_docId).addObject(wing.GetUID(), shape);
        }
    }
    app->getScene()->getViewer()->Update();
 ...
 */
void TIGLCreatorDocument::createShapeTriangulation(const TopoDS_Shape& shape, TopoDS_Compound& compound)
{
    meshShape(shape, TIGLCreatorSettings::Instance().triangulationAccuracy());
    BRep_Builder builder;
...
TiglCPACSConfigurationHandle TIGLCreatorDocument::getCpacsHandle() const
{
    return this->m_cpacsHandle;
}

void TIGLCreatorDocument::updateCpacsConfigurationFromString(const std::string& tixiContent)
{
    if (m_cpacsHandle < 1) {
        LOG(ERROR) << " TIGLCreatorDocument::updateCpacsConfigurationFromString: No model seems to be open.";
        return;
    }
    ...
    if (tiglRet != TIGL_SUCCESS) {
        tixiCloseDocument(tixiHandle);
        m_cpacsHandle = -1;
        displayError(QString("TIGLCreatorDocument::updateCpacsConfigurationFromString: "
                             "<u>tiglOpenCPACSConfiguration</u> returned %1")
